...
public class InsertSqlGenerator extends AbstractSqlGenerator {
    ...
    public PreparedSql prepareSql(final Insert insert, final ConnectionProvider connectionProvider) {
        ...
        final List<org.litebridgedb.db.spi.sql.BindValue> bindValues = new ArrayList<>(insert.rows().size() * columnNames.size());
    ...
    protected PreparedRow prepareRow(final RowValue rowValue, final ConnectionProvider connectionProvider) {
        ...
        final List<org.litebridgedb.db.spi.sql.BindValue> bindValues = new ArrayList<>(rowValue.columns().size());
        ...
        for (final ColumnValue columnValue : rowValue.columns()) {
            ...
            if (convertedValue == null) {
                ...
                valueSpecifiers.add("?");
                bindValues.add(new org.litebridgedb.db.spi.sql.BindValue(convertedValue, column.getDataType()));
            }
        }
