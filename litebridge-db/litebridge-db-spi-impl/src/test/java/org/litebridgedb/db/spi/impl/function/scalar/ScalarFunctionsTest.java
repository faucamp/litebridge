...
class ScalarFunctionsTest {
    ...
    void abs() {
        ...
        // When
        final String sql = abs.toSql(select);
    ...
    void lower() {
        ...
        // When
        final String sql = lower.toSql(select);
    ...
    void upper() {
        ...
        // When
        final String sql = upper.toSql(select);
    ...
    void substring_withLength() {
        ...
        // When
        final String sql = substring.toSql(select);
    ...
    @Test
    void function_withAlias() {
        // Given
        final Column column = new Column(new Table("TEST"), "VAL", "alias");
        final ColumnExpression target = mock(ColumnExpression.class);
        when(target.column()).thenReturn(column);
        when(target.toSql(select)).thenReturn("TEST.VAL");
        final Abs abs = new Abs(target, generator);
        ...
        // When
        final String sql = abs.toSqlWithAlias(select);
    ...
    @Test
    void function_withoutAliasRequestedButAliasPresent() {
        // Given
        final Column column = new Column(new Table("TEST"), "VAL");
        final ColumnExpression target = mock(ColumnExpression.class);
        when(target.column()).thenReturn(column);
