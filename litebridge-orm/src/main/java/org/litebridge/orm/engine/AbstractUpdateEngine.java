...
abstract sealed class AbstractUpdateEngine permits AbstractInsertEngine, DeleteEngine, UpdateEngine {
     ...
     *
     * @param <T>                   The type of the result extending {@link UpdateOpResult}.
     ...
     * @param resultType            The class type of the result to return; must extend {@link UpdateOpResult}.
     * @param litebridgeContext     Context for the statement execution.
     ...
     */
    protected final <T extends Result> T compileAndExecute(final int astCacheKey,
     ...
     * @throws IllegalStateException If the execution of the SQL statement fails, including database or query issues.
     */
    protected final <T extends Result> T execute(final PreparedSql preparedSql, final Class<T> resultType, final LitebridgeContext litebridgeContext) {
